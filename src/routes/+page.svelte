  ...
  const submitHandler: SubmitFunction = () => {
    ...
    return async ({ result, update }) => {
      ...
      if (result.type === "success") {
        const [first, ...rest] = getMessages(
          result.data,
          "Lottery data downloaded successfully.",
        );
      ...
      if (result.type === "failure") {
        toast.error(getMessages(result.data, "Download failed.")[0], {
          id: loadingId,
        });
        return;
      }
      ...
      if (result.type === "redirect") {
        toast.success("Lottery data downloaded successfully.", {
          id: loadingId,
        });
      }
    };
...
>
  ...
  <header class="space-y-3">
    ...
    <p class="max-w-3xl text-zinc-600">
      Download the latest lottery dataset, inspect the SQLite database, and run
      TensorFlow training workflows from one place.
    </p>
  </header>
  ...
  <div class="grid gap-4 md:grid-cols-3">
    ...
    <Card>
      ...
      <p class="mt-2 text-sm text-zinc-600">
        Browse imported entries and verify the ingestion pipeline output.
      </p>
      <div class="mt-4">
        <a
          href="/database"
    ...
    <Card class="border-emerald-200 bg-linear-to-br from-emerald-50 to-white">
      ...
      <div class="mt-4 flex flex-wrap gap-2">
        <a
          href="/training"
          class="inline-flex cursor-pointer items-center rounded-md bg-zinc-900 px-3 py-2 text-sm font-medium text-zinc-50 transition-[transform,colors] duration-100 hover:bg-zinc-800 active:translate-y-px active:scale-[0.98] active:bg-zinc-700"
        >
  ...
  <Card>
    ...
    <div class="mt-3 grid gap-2 text-sm text-zinc-700 sm:grid-cols-2">
      <a
        href="/"
        class="cursor-pointer rounded-md border border-zinc-200 bg-zinc-50 px-3 py-2 hover:bg-zinc-100"
        >/</a
      >
      <a
        href="/database"
